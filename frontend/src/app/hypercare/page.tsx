...
//   usePastSalesData,
//   preferredOrder,
// } from "@/components/PastSales";

import { PastSalesChart, PastSalesTable, usePastSalesData, preferredOrder } from "../../components/PastSales";


export default function HypercarePage() {
  const { todayData, prevData, loading } = usePastSalesData();
  const [selectedChannel, setSelectedChannel] = useState("combo");
...
// </div>;
  ...
  return (
    <div className="px-20 my-10 p-4 text-gray-700">
      <div className="border border-gray-900 shadow-xl p-10 rounded-xl bg-white">
      <PastSalesTable todayData={todayData} prevData={prevData} />

      <div className="mt-6">
        <label className="mr-2 font-semibold">Select Channel:</label>
        <select
          value={selectedChannel}
          onChange={(e) => setSelectedChannel(e.target.value)}
        ...
        >
          <option value="combo">Website</option>
          {preferredOrder.map((ch) =>
            ["Edisons", "Mytopia"].includes(ch) ? null : (
